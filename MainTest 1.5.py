...
class MainWin(QMainWindow,
      ...
      def __init__(self,parent=None):
            ...
            self.tsthread=testthread.TestThread()
            self.tsthread.result.connect(self.tsresult)
      ...
      def on_pushButtonTsStart_clicked(self):

            canbox = self.comboBoxSelectCANbox.currentText()
            canchannel = self.comboBoxSelectCANCH.currentText()
            dbfile = self.DBCdirfile
            if not self.flgtsrun:
                  self.pushButtonTsStart.setStyleSheet("QPushButton{background-color: rgb(255, 0, 0);border-radius: 20px;  border: 2px groove gray;font: 12pt 'Arial'}")
                  self.pushButtonTsStop.setStyleSheet("QPushButton{border-radius: 20px;  border: 2px groove gray;font: 12pt 'Arial'}")
                  self.tsthread.initialize(canbox,
                                        canchannel,
                                        dbfile,
                                        self.flgacrun,
                                        self.dictACFlg,
      ...
      def on_pushButtonTsStop_clicked(self):
          if self.flgacrun:
                  self.pushButtonTsStop.setStyleSheet("QPushButton{background-color: rgb(255, 0, 0);border-radius: 20px;  border: 2px groove gray;font: 12pt 'Arial'}")                  
