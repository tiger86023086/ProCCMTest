...
#  Author : Li Yonghu
#  Build: 19.05.2021
#  Last change: 04.06.2021 Li Yonghu 
#
...
class MainWin(QMainWindow,
      ...
      def __init__(self,parent=None):
            ...
            self.txthread.msgbox.connect(self.displayinfo)
      ...
      @Slot(int)
      def on_horizontalSliderBlwrLvlT_valueChanged(self,value):
            if self.flgacrun:
                  self.initacsig()
      ...
      def display(self,mysigdict):
            if mysigdict != {}:
                  ...
                  self.scrollBarRightTemp.setValue(myltempvalue)
                  self.lineEditTempR.setText(str(myltempvalue))
                  self.scrollBarRightTempT.setValue(myltempvalue)
                  self.lineEditTempRT.setText(str(myltempvalue))                 
      ...
      def initsigval(self):
            self.dictSigVal={'vspd':0,
                             ...
                             'ptcpwr':0,
                             'comppwr':0}
      def displayinfo(self,msgbox):
            for mybox in msgbox:
                  self.listWidget.addItem(mybox)
